...
const MediaPickerModal = ({
    ...
    const lockTab = accept === 'all' ? null : accept;

    const {
        files, loading, uploading, upload,
        search, setSearch, mediaTab, setMediaTab,
        counts, totalCount, filteredCount, isSearchMode, showTabs,
    } = useMediaLibrary({ enabled: open, lockTab });
    ...
    const uploadAccept = accept === 'image'
        ? 'image/*'
        : accept === 'video'
            ? 'video/*'
            : mediaTab === 'video'
        ...
        <>
            ...
            <div className="fixed inset-0 z-60 flex items-center justify-center p-4">
                <div className="bg-neutral-0 dark:bg-neutral-0 rounded-3 shadow-xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden">
                    ...
                    {showTabs && (
                        <div
                            className="flex items-center gap-1 px-6 border-b border-neutral-4 dark:border-neutral-4 shrink-0"
                            role="tablist"
                        >
                            {MEDIA_TABS.map(({ key, label, icon: Icon }) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`
                                            flex items-center gap-2 px-3 py-2.5 -mb-px text-[11px] font-semibold font-poppins
                        ...
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center h-48 gap-3">
                                <div className="w-12 h-12 rounded-2 bg-neutral-2 flex items-center justify-center">
                                    {mediaTab === 'video'
                                        ? <Film size={22} className="text-neutral-4" />
                                ...
                                <p className="text-xs font-poppins text-neutral-5">
                                    {search || isSearchMode
                                        ...
                                        : mediaTab === 'video'
                                            ? 'Aucune vidéo disponible — uploadez-en une !'
                                            : 'Aucune image disponible — uploadez-en une !'}
                                </p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
                                {files.map(file => {
                                    const type = getMediaFileType(file.file);
                                        ...
                                        >
                                            <div className="w-full h-full bg-neutral-2">
                                                {type === 'image' ? (
                                                    ...
                                                    <VideoFileThumbnail src={file.file} iconSize={20} />
                                                ) : (
                    ...
                    <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-neutral-4 dark:border-neutral-4 shrink-0">
                        <p className="text-[11px] font-poppins text-neutral-5">
                            {loading
                                ? '…'
                                : showTabs
                                    ? `${counts.image} image${counts.image !== 1 ? 's' : ''} · ${counts.video} vidéo${counts.video !== 1 ? 's' : ''}`
                                    : `${filteredCount} ${mediaTab === 'video' ? 'vidéo' : 'image'}${filteredCount !== 1 ? 's' : ''} · ${totalCount} au total`}
                        </p>
